...
const SERVICE_ROLE = new Set([
  ...
  "stripe_events",
  // Append-only owner/venue T&C acceptance ledger (B3) — inserted only by the claim +
  // paid-checkout routes via the service-role client (recordOwnerTermsAcceptance);
  // self/admin-read RLS, no insert policy, UPDATE + DELETE blocked by triggers (085).
